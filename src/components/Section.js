...
export default function Section(props) {
    ...
    const updatequote = async () => {
     ...
     console.log(setti.value);
    //  let url = `/apii/api/random/${props.url}=${setti.value}`;
     let url = `https://animechan.xyz/api/random/${props.url}=${setti.value}`;
  ...
  return (
   <>
        ...
        <h1  className=''>Get Random quotes by Anime {props.head} </h1>  
         <input id="searchar"type="text" placeholder={props.head} />
        <button className='btn' onClick={updatequote}> Get a quote</button>
        </div>
        <div className={props.box}>
            <div className='contentbox'> <h2>"{mquote}"</h2>
        <div id='loader'>
            <img style={{width:'40px',display:'none'}} src={ghost} alt="loading" />
        </div>
        </div>
